        ...
        <>
            <div className='template_card border-[1px] border-divider-main max-w-[370px] w-full'>
                <Link href={`/product/${items.id}`} className='relative'>
                    <Image src={`http://localhost:7777/upload/${items.fullimages[0]?.filename}`} width={370} height={278} alt="Image Error" className=''></Image>
                    ...
                    <div className='flex justify-between mt-3 gap-2 flex-wrap items-start'>
                        <div className='flex items-center gap-2'>
                            ...
                            <div className='flex gap-[10px] items-center'>
                                <p className='medium-heading text-[12px]'> <span className='font-open-sans text-[12px] text-light-text leading-5 font-normal pr-1'>By</span> <Link href="/" className=''>{items.companyName}</Link></p>
                                <Link href="/" className='medium-heading text-[12px]'>{items.templateType}</Link>
                            </div>
                        </div>
