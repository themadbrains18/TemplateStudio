    ...
    return (
        <>
            <Link href={`/product/${props.data.id}`} onClick={()=>{props.closeSideMenu()}}>
                <div className='border-[0.5px] border-divider-main  mx-auto w-full'>
                    <Image src={`http://localhost:7777/upload/${props.data.sliderimages[0]?.filename}`} width={142} height={92} alt="Image Error" className='w-full'></Image>
